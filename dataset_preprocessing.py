...
def get_files():
    """ Download the opencell files from the `files` list above that we need """
    for file in files:
        file_path = DATA_DIR / file
        if not os.path.exists(file_path):
            headers = {'Referer': url_base}
            response = requests.get(f"{url_base}/{file}", headers=headers)
...
def get_protein_sequence_representation():
    ...
    def get_uniprot_sequence(uniprot_id):
        ...
        if response.ok:
            ...
            print(
                f"Error retrieving data for UniProt ID {uniprot_id}: {response.status_code}"
            )
            return None
    ...
    def extract_sequence_from_fasta(fasta_str):
        ...
        sequence_only = ''.join(line for line in lines
...
def get_sequence_representation_language():
    ...
    for i, (_, seq) in enumerate(data):
        sequence_representations.append(
            token_representations_all[i, 1:len(seq) + 1].mean(0))
...
def save_preprocessing_datasets():
    def compress_files(file_paths, output_zip_file):
        """
	    From ChatGPT
	    ...
	    output_zip_file (str): The path of the output ZIP file.
	    """
        with zipfile.ZipFile(output_zip_file, 'w',
                             zipfile.ZIP_DEFLATED) as zipf:
            for file in file_paths:
                # Add file to the zip file
                # os.path.basename(file) is used to remove the directory path
                zipf.write(file, os.path.basename(file))

    fnames = [
        "protein_sequences.pt", "prot_pubmed_counts.pt",
        "sequence_representations.pt", "opencell_image_embedding.pt"
    ]
    file_paths = [f"data_preprocessed/{f}" for f in fnames]
    output_zip_file = f"data_preprocessed/data_preprocessed.zip"
    compress_files(file_paths, output_zip_file)
