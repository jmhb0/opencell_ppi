...
class OpencellPPI(InMemoryDataset):
    ...
    def __init__(
        ...
        if self.features_type == "dummy":
        	...
        	self._data.x = self._data.features_sequence 
        elif self.features_type == "image":
    ...
    def process(self):
        ...
        self.y_loc_nuclear, self.y_loc = self.get_node_labels_localization(
            df_loc)

        # get the features
        self.dummy_features = torch.ones((len(self.prots), 5))
    ...
    def get_node_labels_localization(self, df_loc, threshold_include=40):
        """ 
        ...
        uniq_locs, uniq_cnts = np.unique(all_locs_flat, return_counts=True)
        uniq_locs = uniq_locs[uniq_cnts >= threshold_include]
        uniq_locs = np.sort(uniq_locs)  # get unique stuff
        self.uniq_locs = [s for s in uniq_locs
                          if s != '']  # remove empty entries
    ...
    def get_features_sequence(self):
        ...
        self.sequences = [
            lookup_protein_sequences.get(p, "") for p in self.prots
        ]

        file_sequence_representations = f"{self.root}/raw/sequence_representations.pt"
        self.sequence_representations = torch.load(
            file_sequence_representations)

        return self.sequence_representations

    def get_features_image(self):
        """ """
        file_image_representations = f"{self.root}/raw/opencell_image_embedding.pt"
        self.image_representations = torch.load(
            file_image_representations)
        return self.image_representations

    def get_protein_ordering(self):
        """ 
		Assign a rank ordering, and a normalized rank orderings to each of protein.
		This is based on citation counds,  that we import that was previously 
		calculated in the `dataset_preprocessing.py` func. 
		"""
        file_pubmed_cites = f"{self.root}/raw/prot_pubmed_counts.pt"
        lookup_pubmed_cites = torch.load(file_pubmed_cites)
        self.pubmed_cites = torch.tensor(
            [lookup_pubmed_cites[p] for p in self.prots])
